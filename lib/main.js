...
function getJson(config, callback, js) {
	...
	https.get(config, function(res) {
		...
		res.on('end', function () {
			if(js == 11)
				...
				callback(JSON.parse(body), port,body);
		});
	});
}
...
function getSaveStatus(track) {
	config = copyConfig();
	config.host = 'api.spotify.com';
	config.path = '/v1/me/tracks/contains=' + track + '?oauth=' + oauth + '&csrf=' + csrf ;
	config.port = 443;
...
function grabTokens() {
	...
	getJson(config, function(data) { console.log(data); csrf = data.token; });
	config.host = 'open.spotify.com';
	config.path = '/token';
	config.port = 443;
	getJson(config, function(data) { console.log(data); oauth = data.t; });
